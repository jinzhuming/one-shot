...
0. 先阅读项目根目录 `AGENTS.md`；它是本仓库的具体防回归约束。
1. 优先用系统控件、系统字体、语义色、SF Symbols、系统 material；不要自绘一套 iOS/网页风皮肤。
2. 菜单栏 extra 点按后出 **菜单**，不要用 popover 塞复杂设置。
3. 用户可见文案走本地化（开发语言 `zh-Hans`），不要在代码里新增长硬编码中英混写；改完运行 `make validate-localization`。
4. 每个可点击控件有 `help` / accessibility 名称；自定义 `NSView` 补 VoiceOver。
5. `Support/Info.plist` 可以包含 Xcode 构建宏，但打包产物里的 `Info.plist` **不得**含未展开的 `$(...)`。
...
- `make verify`、`make validate-localization` 已通过；若改了 `project.yml`，先运行 `make generate`
